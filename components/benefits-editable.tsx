...
type BenefitsProps = Partial<BenefitsBlock> & {
  blockType?: string;
  id?: string;
};
...
const iconMap: Record<string, React.ReactNode> = {
  ...
  sync: <RealtimeSyncIcon className="size-6 text-bg-dark dark:text-brand" />,
  rocket: <RocketIcon className="size-6 text-bg-dark dark:text-brand" />,
  shield: <ShieldIcon className="size-6 text-bg-dark dark:text-brand" />,
  brain: <ReuseBrainIcon className="size-6 text-bg-dark dark:text-brand" />,
  cog: <ScreenCogIcon className="size-6 text-bg-dark dark:text-brand" />,
  bell: <BellIcon className="size-6 text-bg-dark dark:text-brand" />,
};
...
export const Benefits = (props: BenefitsProps = {}) => {
  const {
    ...
    subheading = "Every insight ties directly to revenue impact. We optimize for $$$ in your bank account, not impressions on a dashboard. Built by AI researchers who deployed systems at Google and fintech founders who scaled companies to 8-figure exits.",
    benefits = [
      {
      ...
      <div className="mt-20 grid grid-cols-1 gap-4 md:grid-cols-3">
        <div className="grid auto-rows-fr grid-cols-1 gap-4">
          {benefitsList.slice(0, 3).map((benefit, index) => (
            <Card
              ...
              icon={iconMap[benefit.icon || "graph"] || iconMap.graph}
            />
          ))}
        </div>
        <MiddleCard />
        <div className="grid auto-rows-fr grid-cols-1 gap-4">
          {benefitsList.slice(3, 6).map((benefit, index) => (
            <Card
              key={benefit.title || `benefit-${index + 3}`}
              title={benefit.title || ""}
              description={benefit.description || ""}
              icon={iconMap[benefit.icon || "graph"] || iconMap.graph}
            />
