...
export const Hero = () => {
  return (
    <Container className="border-divide border-x px-4 pt-10 pb-10 md:pt-20 md:pb-20">
      {/* Two Column Layout */}
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-12 items-center">
        ...
        <div className="flex flex-col justify-center space-y-8">
          {/* Badge */}
          <div>
            <Badge text="For Financial Services Leaders" />
          </div>

          {/* Main Heading */}
          <div className="space-y-6">
            <Heading className="text-left leading-tight">
              Understand how you <span className="text-brand">lose clients</span> on{" "}
              <span className="inline-flex items-center gap-2 text-bg-dark dark:text-white">
                <svg
            ...
            <SubHeading className="text-left leading-relaxed">
              Optimize your presence where 57% of consumers search before buying.
            </SubHeading>
          </div>
          ...
          <div className="space-y-3 border-l-4 border-brand pl-4">
            <p className="text-base font-medium text-bg-dark dark:text-white">
              <span className="text-brand font-bold">20%+ of traffic</span> now
              comes from ChatGPT for leading financial services companies
            </p>
        ...
        <div className="flex flex-col space-y-6">
          {/* Alex Persona - ChatGPT Style */}
          <div className="overflow-hidden rounded-2xl bg-white shadow-xl dark:bg-[#212121]">
            ...
            <div className="bg-gray-50 p-4 dark:bg-[#2f2f2f]">
              <div className="flex gap-4">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gray-200 text-sm font-medium text-gray-700 dark:bg-[#565869] dark:text-gray-300">
                  A
                </div>
...
const AnimatedChat = () => {
  ...
  const handleSendMessage = () => {
    if (inputText.trim()) {
      const newMessages = [
        ...
        {
          role: "assistant",
          content: "This is a demo - the chat shows how prospects research on ChatGPT.",
        },
      ];
      setChat(newMessages);
  ...
  return (
    <motion.div
      ...
      className="relative overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-[#212121]"
    >
      {/* Chat Header - Style ChatGPT exact */}
      <div className="border-b border-gray-200 bg-white px-4 py-3 dark:border-[#424242] dark:bg-[#212121]">
      ...
      {/* Chat Messages - Alternating backgrounds like ChatGPT */}
      <div
        ref={setChatContainerRef}
        className="flex max-h-[400px] min-h-[250px] flex-col overflow-y-auto pb-20"
        style={{
          scrollbarWidth: "thin",
        }}
      >
        {chat.slice(0, visibleMessages).map((message, index) => (
          <motion.div
            ...
            className={message.role === "assistant" ? "bg-gray-50 dark:bg-[#2f2f2f]" : "bg-white dark:bg-[#212121]"}
          >
            {message.role === "user" ? (
              <UserMessage
      ...
      <div className="absolute inset-x-0 bottom-0 border-t border-gray-200 bg-white p-4 dark:border-[#424242] dark:bg-[#212121]">
        <div className="mx-auto flex max-w-3xl items-center gap-3 rounded-full border border-gray-300 bg-white px-4 py-3 shadow-sm dark:border-[#4e4e4e] dark:bg-[#2f2f2f]">
          {/* Plus Button */}
          <button className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-[#3e3e3e]">
          ...
          <input
            ...
            className="flex-1 border-none bg-transparent text-base placeholder-gray-400 focus:outline-none dark:text-white dark:placeholder-gray-500"
          ...
          <button
            ...
            className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full transition-all ${
              inputText.trim()
                ...
                : "bg-gray-200 text-gray-400 dark:bg-[#4e4e4e] dark:text-gray-600"
            }`}
...
// Helper function to parse content and render with citation links
const renderContentWithLinks = (text: string) => {
  const parts: React.ReactNode[] = [];
  let lastIndex = 0;

  // Regex to match [linkN] or [text linkN] patterns
  const linkPattern = /\[([^\]]+)\]/g;
  let match;
  ...
  while ((match = linkPattern.exec(text)) !== null) {
    ...
    // Parse the links inside brackets
    const linksText = match[1];
    const links = linksText.split(',').map(l => l.trim());

    // Render each link as a citation with the actual link text - ChatGPT style badges
    links.forEach((link, idx) => {
      parts.push(
        <span
          key={`${match.index}-${idx}`}
          className="mx-1 inline-flex items-center rounded-full border border-gray-300 bg-white px-2 py-0.5 text-xs font-medium text-gray-700 hover:bg-gray-50 dark:border-[#565869] dark:bg-[#2f2f2f] dark:text-gray-300 dark:hover:bg-[#3e3e3e]"
        >
          {link}
        </span>
      );
...
const AssistantMessage = ({
  ...
  const textToRender = isActive ? displayText : content;
  ...
  return (
    <div className="flex gap-4 px-4 py-6 md:px-6">
      ...
      <div className="flex-1 pt-1">
        <div className="whitespace-pre-line text-base text-gray-900 dark:text-gray-100">
          {renderContentWithLinks(textToRender)}
          {isActive && !isComplete && <span className="animate-pulse">|</span>}
        </div>
      </div>
    </div>
