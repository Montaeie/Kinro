...
import type { VisionFunnelBlock } from "@/payload-types";

type VisionFunnelProps = Partial<VisionFunnelBlock> & {
  blockType?: string;
  id?: string;
};

export const VisionFunnel = (props: VisionFunnelProps = {}) => {
  const {
    badge = "Vision du Futur",
    heading = "Service subscription journey is shifting",
    subheading = "→ From recommendation to fully integrated funnel",
    bottomText = "Before: No visibility or control. Now: Partial integration. Then: Full funnel ownership with Satsu.",
  } = props;
      ...
      <div className="px-4 py-20 md:px-8 md:py-20">
        ...
        <div className="mt-12 text-center">
          <p className="text-sm text-gray-600 dark:text-gray-400">
            {bottomText.split('.').map((sentence, idx) => {
              const trimmed = sentence.trim();
              if (!trimmed) return null;

              // Parse the text to add proper styling
              if (trimmed.includes('Before:')) {
                return (
                  <span key={idx}>
                    <span className="font-semibold text-bg-dark dark:text-white">Before:</span> {trimmed.split('Before:')[1]}.{' '}
                  </span>
                );
              } else if (trimmed.includes('Now:')) {
                return (
                  <span key={idx}>
                    <span className="font-semibold text-bg-dark dark:text-white">Now:</span> {trimmed.split('Now:')[1]}.{' '}
                  </span>
                );
              } else if (trimmed.includes('Then:')) {
                return (
                  <span key={idx}>
                    <span className="font-semibold text-brand">Then:</span> {trimmed.split('Then:')[1]}.
                  </span>
                );
              }
